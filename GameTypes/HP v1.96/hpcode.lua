--Changes since v1.96
-- All bugs from v1.95 fixed
-- 1 Grenade is now given initially on spawn, and the delay timer gives you another 3 instead of 2 afterwards.
      ...
      if global.number[3] == 0 then
         ...
         for each player do
            if current_player.number[1] != 1 and current_player.number[0] == 1 then 
               --current_player.score += script_option[4] --give score to all players on the winning team
			   -- has a bug atm where if zombies win, the LMS also gets score
            end
         end
         game.end_round()
      end
...
if game.round_timer.is_zero() and game.round_time_limit > 0 then 
   ...
   if not global.number[3] == 0 then
      ...
      for each player do
         if current_player.number[0] == 0 then 
            --current_player.score += script_option[5] --give score to all players on the winning team
         end
      end
      game.end_round()
   end
